...
CONTAINER_NAME="${CONTAINER_NAME:-maps-schema-pipeline-builder-runtime-smoke}"
TOPIC_NAMESPACE="${TOPIC_NAMESPACE:-/skill/maps-schema-pipeline-builder-runtime-smoke/runtime}"
...
TMP_DIR="$(mktemp -d /tmp/maps-schema-pipeline-builder-runtime-smoke.XXXXXX)"
...
if [[ "${REQUIRE_MQTT_RUNTIME}" == "1" ]]; then
  ...
  echo "maps-schema-pipeline-builder runtime smoke PASS"
  exit 0
fi
...
if timeout 3 bash -lc "echo > /dev/tcp/127.0.0.1/${MQTT_PORT}" 2>/dev/null; then
  ...
  CORR="maps_schema_pipeline_builder_runtime_smoke-$(date +%s)"
  timeout 8 mosquitto_sub -V mqttv311 -h 127.0.0.1 -p "${MQTT_PORT}" -t "${TOPIC}" -C 1 "${AUTH[@]}" >/tmp/maps-schema-pipeline-builder-runtime-smoke.out 2>/tmp/maps-schema-pipeline-builder-runtime-smoke.err &
  SPID=$!
  sleep 1
  mosquitto_pub -V mqttv311 -h 127.0.0.1 -p "${MQTT_PORT}" -t "${TOPIC}" -m "${CORR}" "${AUTH[@]}"
  wait "${SPID}"
  rg -n "${CORR}" /tmp/maps-schema-pipeline-builder-runtime-smoke.out >/dev/null
else
  echo "MQTT listener not available on ${MQTT_PORT}; startup/listener runtime check passed without maps_schema_pipeline_builder_runtime_smoke path."
fi
