...
CONTAINER_NAME="${CONTAINER_NAME:-maps-deployment-packager-runtime-smoke}"
TOPIC_NAMESPACE="${TOPIC_NAMESPACE:-/skill/maps-deployment-packager-runtime-smoke/runtime}"
...
TMP_DIR="$(mktemp -d /tmp/maps-deployment-packager-runtime-smoke.XXXXXX)"
...
if [[ "${REQUIRE_MQTT_RUNTIME}" == "1" ]]; then
  ...
  echo "maps-deployment-packager runtime smoke PASS"
  exit 0
fi
...
if timeout 3 bash -lc "echo > /dev/tcp/127.0.0.1/${MQTT_PORT}" 2>/dev/null; then
  ...
  CORR="maps_deployment_packager_runtime_smoke-$(date +%s)"
  timeout 8 mosquitto_sub -V mqttv311 -h 127.0.0.1 -p "${MQTT_PORT}" -t "${TOPIC}" -C 1 "${AUTH[@]}" >/tmp/maps-deployment-packager-runtime-smoke.out 2>/tmp/maps-deployment-packager-runtime-smoke.err &
  SPID=$!
  sleep 1
  mosquitto_pub -V mqttv311 -h 127.0.0.1 -p "${MQTT_PORT}" -t "${TOPIC}" -m "${CORR}" "${AUTH[@]}"
  wait "${SPID}"
  rg -n "${CORR}" /tmp/maps-deployment-packager-runtime-smoke.out >/dev/null
else
  echo "MQTT listener not available on ${MQTT_PORT}; startup/listener runtime check passed without maps_deployment_packager_runtime_smoke path."
fi
