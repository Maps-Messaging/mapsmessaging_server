...
```bash
rg -n "NetworkManager|DestinationManager|routing|SchemaManager" *.yaml
...
```bash
bash skills/maps-artifact-execution-smoke-harness/scripts/run_matrix.sh --channel release --artifact-dir <repo-root> --base-mqtt-port 2883 --base-http-port 28080
```

Snapshot example (latest requested snapshot version):

```bash
bash skills/maps-artifact-execution-smoke-harness/scripts/run_matrix.sh --channel snapshot --version 4.3.1-snapshot --arch arm64 --platform linux/arm64 --artifact-dir <repo-root> --base-mqtt-port 3883 --base-http-port 38080
```

x86 release example:

```bash
bash skills/maps-artifact-execution-smoke-harness/scripts/run_matrix.sh --channel release --arch amd64 --platform linux/amd64 --artifact-dir <repo-root> --base-mqtt-port 2883 --base-http-port 28080
```

Auth-enabled MQTT example:

```bash
bash skills/maps-artifact-execution-smoke-harness/scripts/run_matrix.sh --channel snapshot --version 4.3.1-snapshot --arch arm64 --platform linux/arm64 --artifact-dir <repo-root> --mqtt-username maps-user --mqtt-password maps-pass --base-mqtt-port 4883 --base-http-port 48080
```

Listener-only example for artifacts with non-MQTT flows:

```bash
bash skills/maps-artifact-execution-smoke-harness/scripts/run_artifact_smoke.sh --image mapsmessaging/server_daemon:latest --artifact-dir <repo-root> --skip-mqtt --required-listeners 8080 --platform linux/amd64 --mqtt-port 5883 --http-port 58080 --force-clean
```
