...
public class SimpleStreamTest {
  ...
  private void checkStream(ObjectReader sor) throws IOException {
    Assert.assertEquals(byteVal, sor.readByte());
    Assert.assertEquals(shortVal, sor.readShort());
    Assert.assertEquals(intVal, sor.readInt());
    Assert.assertEquals(longVal, sor.readLong());
    Assert.assertEquals(floatVal, sor.readFloat());
    Assert.assertEquals(doubleVal, sor.readDouble());
    Assert.assertEquals(nullString, sor.readString());
    Assert.assertEquals(emptyString, sor.readString());
    Assert.assertEquals(string, sor.readString());
    Assert.assertNull(sor.readByteArray());
    Assert.assertEquals(0, sor.readByteArray().length);
    byte[] tmp = sor.readByteArray();
    for(int x=0;x<tmp.length;x++){
      Assert.assertEquals(byteArray[x], tmp[x]);
    }
  }
}
