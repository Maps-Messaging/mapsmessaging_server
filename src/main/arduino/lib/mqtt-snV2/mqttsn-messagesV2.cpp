...
MQTTSN::MQTTSN(RHReliableDatagram *loraConnection) : 
...
_response_timer(0)
{
    ...
    this->loraConnection = loraConnection;    
}
...
//#ifdef USE_LORA
void MQTTSN::parse_rf12(uint8_t* buf, uint8_t len) {
    memcpy(response_buffer, (const void*)buf, len);
    dispatch();
}
//#endif
...
void MQTTSN::loop(){
  ...
  if (loraConnection->recvfromAck(buf, &len, &from)) {
    if(from == _gateway_id){
      parse_rf12(buf, len);
      waiting_for_response = false;
    }
...
void MQTTSN::send_message() {
    ...
    this->loraConnection->sendtoWait(message_buffer, hdr->length, _gateway_id);
...
void MQTTSN::setLoraConnection(RHReliableDatagram* lora){
    this->loraConnection = lora;
}
