...
public class SSLHandShakeManagerImpl implements SSLHandshakeManager {
  ...
  SSLHandShakeManagerImpl(SSLEndPoint sslEndPointImpl) {
    this.sslEndPointImpl = sslEndPointImpl;
    handshakeBufferIn =
        ByteBuffer.allocate(sslEndPointImpl.sslEngine.getSession().getApplicationBufferSize());
    handshakeBufferOut =
        ByteBuffer.allocate(sslEndPointImpl.sslEngine.getSession().getApplicationBufferSize());
  }
  ...
  public boolean handleSSLHandshakeStatus() throws IOException {
    ...
    logger.log(
        LogMessages.SSL_HANDSHAKE_ENCRYPTED,
        handshakeBufferIn.position(),
        handshakeBufferIn.limit());
    sslEndPointImpl.handshakeManager =
        new SSLHandshakeManagerFinished(); // All done, no longer required
    return false;
  }
