...
public class OrbcommProtocol extends Protocol {
  ...
  public OrbcommProtocol(EndPoint endPoint, Packet packet) throws LoginException, IOException {
    super(endPoint,  endPoint.getConfig().getProtocolConfig("orbcomm"));

    if (packet != null) {
      packet.clear();
    }
    SessionContextBuilder sessionContextBuilder = new SessionContextBuilder("OrbComm" + endPoint.getId(), new ProtocolClientConnection(this));
    ...
    ConfigurationProperties configurationProperties = ConfigurationManager.getInstance().getProperties("orbcomm");
  ...
  public boolean processPacket(Packet packet) throws IOException {
    while (packet.hasRemaining()) {
      // ToDo Process response to AT command
    }
    endPoint.register(SelectionKey.OP_READ, selectorTask.getReadTask());
    return true;
  }
  ...
  public String getSessionId() {
    return "OrbComm_" + endPoint.getName();
  }
