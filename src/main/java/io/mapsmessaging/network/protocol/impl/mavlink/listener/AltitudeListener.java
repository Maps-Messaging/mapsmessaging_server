...
import io.mapsmessaging.network.protocol.impl.mavlink.packet.AltitudePacket;
...
public class AltitudeListener implements Listener {
  ...
  public static final int LISTENER_ID = ALTITUDE;

  private final TwinManager twinManager;

  public AltitudeListener(TwinManager twinManager) {
  ...
  public void handle(String twinId,
    ...
    twinManager.updateTwin(twinId, twin -> {
      ...
      GeoPosition geoPosition = drone.getGeoPosition();
      if (geoPosition == null) {
        geoPosition = new GeoPosition();
      }

      if (!Double.isNaN(packet.getAltitudeAmslMeters())) {
        geoPosition.setAltitudeMslMeters(packet.getAltitudeAmslMeters());
      }
