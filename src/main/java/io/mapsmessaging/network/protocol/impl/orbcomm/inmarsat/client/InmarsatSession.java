...
public final class InmarsatSession {
  ...
  private InmarsatSession(URI base, String clientId, String clientSecret, HttpClient http, Gson gson, Duration errorCacheTtl) {
    this.base = base;
    this.clientId = clientId;
    this.clientSecret = clientSecret;
    this.http = Objects.requireNonNullElseGet(http, () -> HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(20)).build());
    this.gson = Objects.requireNonNullElseGet(gson, Gson::new);
    this.errorCacheTtl = Objects.requireNonNullElse(errorCacheTtl, Duration.ofHours(24));
  ...
  // Factories
  public static InmarsatSession of(String baseUrlWithVersion, String clientId, String clientSecret, HttpClient http, Gson gson) {
    return new InmarsatSession(URI.create(ensureTrailingSlash(baseUrlWithVersion)), clientId, clientSecret, http, gson, Duration.ofHours(24));
  }

  public static InmarsatSession of(String baseUrlWithVersion, String clientId, String clientSecret) {
    return of(baseUrlWithVersion, clientId, clientSecret, defaultHttpClient(), defaultGson());
  }

  // Optional: custom TTL
  public static InmarsatSession of(String baseUrlWithVersion, String clientId, String clientSecret, Duration errorCacheTtl) {
    return new InmarsatSession(URI.create(ensureTrailingSlash(baseUrlWithVersion)), clientId, clientSecret, defaultHttpClient(), defaultGson(), errorCacheTtl);
  }
  ...
  public MailboxSession openMailbox(String mailboxId, String mailboxPassword) {
  ...
  public static void main(String[] args){
    InmarsatSession inmarsatSession = InmarsatSession.of(
        "https://api.inmarsat.com/iotMessaging/v1",
        "0oah7vv2joaSE8O5D0i7",
        "XFfJ3z6b4_QvZBksU7-dfCIEO7zCFUwJDSf15fDoGM5b9A6TZxJgwLYHo-ZPUycm");

    inmarsatSession.refreshErrors();
    for (Map.Entry<Integer, ErrorDef> errorDefEntry: inmarsatSession.errorIndexRef.get().entrySet()){
      System.err.println(errorDefEntry.getValue());
    }

    MailboxSession mbSession = inmarsatSession.openMailbox("c3052ac1-a7f8-4101-b336-5aef3b6c5af6", "Gq7=@n!tfy=/");
    System.err.println(mbSession.getMailbox());

    for(DeviceInfo device: mbSession.listDevices()){
