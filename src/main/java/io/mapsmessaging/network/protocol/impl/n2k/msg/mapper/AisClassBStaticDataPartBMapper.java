...
public class AisClassBStaticDataPartBMapper {

  private final AisClassBEmitterConfig config;

  public AisClassBStaticDataPartBMapper(AisClassBEmitterConfig config) {
    this.config = config;
  }

  public Optional<AisClassBStaticDataPartBReport> map(DroneTwin droneTwin) {
    ...
    report.setVendorId(AisMappingSupport.resolveVendorId(config.getVendorId()));
    report.setCallsign(AisMappingSupport.resolveCallsign(droneTwin, config.getCallsign()));
    ...
    report.setMothershipUserId(config.getMothershipUserId());
