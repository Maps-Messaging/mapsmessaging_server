/*
 *
 *  Copyright [ 2020 - 2024 ] Matthew Buckton
 *  Copyright [ 2024 - 2026 ] MapsMessaging B.V.
 *
 *  Licensed under the Apache License, Version 2.0 with the Commons Clause
 *  (the "License"); you may not use this file except in compliance with the License.
 *  You may obtain a copy of the License at:
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *      https://commonsclause.com/
 *
 *  Unless required by applicable law or agreed to in writing, software
 *  distributed under the License is distributed on an "AS IS" BASIS,
 *  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *  See the License for the specific language governing permissions and
 *  limitations under the License.
 */

package io.mapsmessaging.tools.config.yaml;

import java.util.Locale;
import java.util.List;
import java.util.Map;

public final class YamlValueFormatter {

  public String formatScalar(Object value) {
    switch (value) {
      case null -> {
        return "null";
      }
      case Boolean booleanValue -> {
        return booleanValue ? "true" : "false";
      }
      case Double doubleValue -> {
        if (doubleValue.isInfinite() && doubleValue == Math.rint(doubleValue)) {
          long longValue = doubleValue.longValue();
          return String.valueOf(longValue);
        }
        return stripTrailingZeros(doubleValue);
      }
      case Float floatValue -> {
        if (Float.isFinite(floatValue) && floatValue == Math.rint(floatValue)) {
          long longValue = floatValue.longValue();
          return String.valueOf(longValue);
        }
        return stripTrailingZeros(floatValue.doubleValue());
      }
      case Number numberValue -> {
        return numberValue.toString();
      }
      case String stringValue -> {
        return quoteIfNeeded(stringValue);
      }
      case Map<?, ?> map -> {
        return "{}";
      }
      case List<?> objects -> {
        return "[]";
      }
      default -> {
      }
    }

    return quoteIfNeeded(String.valueOf(value));
  }

  private String stripTrailingZeros(double value) {
    String text = Double.toString(value);
    if (text.indexOf('E') >= 0 || text.indexOf('e') >= 0) {
      return text;
    }
    if (!text.contains(".")) {
      return text;
    }
    while (text.endsWith("0")) {
      text = text.substring(0, text.length() - 1);
    }
    if (text.endsWith(".")) {
      text = text.substring(0, text.length() - 1);
    }
    return text;
  }

  private String quoteIfNeeded(String value) {
    if (value.isEmpty()) {
      return "\"\"";
    }

    boolean needsQuotes =
        value.startsWith(" ")
            || value.endsWith(" ")
            || value.contains(": ")
            || value.contains("#")
            || value.contains("\n")
            || value.startsWith("-")
            || looksLikeYamlLiteral(value);

    if (!needsQuotes) {
      return value;
    }

    String escaped = value
        .replace("\\", "\\\\")
        .replace("\"", "\\\"");
    return "\"" + escaped + "\"";
  }

  private boolean looksLikeYamlLiteral(String value) {
    String lower = value.toLowerCase(Locale.ROOT);
    if (lower.equals("null") || lower.equals("true") || lower.equals("false")) {
      return true;
    }

    try {
      Double.parseDouble(value);
      return true;
    } catch (Exception ignored) {
      return false;
    }
  }
}
