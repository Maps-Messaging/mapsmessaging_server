...
public class EndPointConfigFactory {
  ...
  public static boolean update(EndPointServerConfigDTO orig, EndPointServerConfigDTO upd) {
    ...
    if (!orig.getName().equals(upd.getName())) { orig.setName(upd.getName()); hasChanged = true; }
    if (!orig.getAuthenticationRealm().equals(upd.getAuthenticationRealm())) { orig.setAuthenticationRealm(upd.getAuthenticationRealm()); hasChanged = true; }
    if (orig.getBacklog() != upd.getBacklog()) { orig.setBacklog(upd.getBacklog()); hasChanged = true; }
    if (orig.getSelectorTaskWait() != upd.getSelectorTaskWait()) { orig.setSelectorTaskWait(upd.getSelectorTaskWait()); hasChanged = true; }
    if (orig.getEndPointConfig() instanceof Config endpointConfig && endpointConfig.update(upd.getEndPointConfig())) hasChanged = true;
    if (orig.getSaslConfig() != null && ((Config) orig.getSaslConfig()).update(upd.getSaslConfig())) hasChanged = true;
    return hasChanged;
  }
  ...
  public static void pack(ConfigurationProperties config, EndPointServerConfigDTO server) {
    ...
    config.put("protocols", packProtocolConfig(server));
    if (server.getEndPointConfig() instanceof Config endpointConfig) config.put("endPoint", endpointConfig.toConfigurationProperties());
    if (server.getSaslConfig() != null) config.put("saslConfig", ((Config) server.getSaslConfig()).toConfigurationProperties());
  }
  ...
  public static void unpack(ConfigurationProperties config, EndPointServerConfigDTO server) {
    ...
    if (server.getUrl() == null) server.setUrl(config.getProperty("protocol") + "://localhost:0");
    ...
    if (saslConfiguration != null) server.setSaslConfig(new SaslConfig(saslConfiguration));
    ...
    if (config.containsKey("protocol")) {
      ...
      if (obj instanceof List) loadSpecificProtocols(server, (List<ConfigurationProperties>) obj);
      else loadSpecificProtocols(server, List.of((ConfigurationProperties) obj));
    } else {
  ...
  private static void loadSpecificProtocols(EndPointServerConfigDTO server, List<ConfigurationProperties> protocolConfig) {
    ...
    for (ConfigurationProperties prop : protocolConfig) protocolConfigs.add(createProtocolConfig(prop.getProperty("type"), prop));
  ...
  private static void loadDefaultProtocols(ConfigurationProperties config, EndPointServerConfigDTO server, String protocol) {
    ...
    for (String protocolName : protocolArray) {
      ...
      if (protocolName.equalsIgnoreCase("nmea")) protocolName = "NMEA-0183";
      protocolList.add(protocolName);
    }
    List<ProtocolConfigDTO> protocolConfigs = new ArrayList<>();
    for (String protocolName : protocolList) protocolConfigs.add(createProtocolConfig(protocolName.trim(), config));
    server.setProtocolConfigs(protocolConfigs);
  }
  ...
  private static List<ConfigurationProperties> packProtocolConfig(EndPointServerConfigDTO server) {
    ...
    for (ProtocolConfigDTO protocolConfigDTO : server.getProtocolConfigs()) {
      if (protocolConfigDTO instanceof Config protocolConfig) protocolConfigs.add(protocolConfig.toConfigurationProperties());
    }
  ...
  private static EndPointConfigDTO createEndPointConfig(String url, ConfigurationProperties properties) {
    ...
    if (u.startsWith("shm") || u.startsWith("rdma")) return new MemoryConfig(properties);
    ...
    if (u.startsWith("satellite")) return new EndPointConfigDTO("satellite");
    return null;
  }
  ...
  private static ProtocolConfigDTO createProtocolConfig(String protocol, ConfigurationProperties config) {
    ...
    return switch (p) {
      case "maps" -> new MapsConfig(config);
      case "mqtt-v5", "mqtt-v3", "mqtt" -> new MqttConfig(config);
