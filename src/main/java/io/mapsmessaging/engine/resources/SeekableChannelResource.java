...
public class SeekableChannelResource extends MapBasedResource {
  public SeekableChannelResource(String name, String mapped) throws IOException {
    ...
    setStore(new WeakReferenceCacheStorage<>(Objects.requireNonNull(StorageFactoryFactory.getInstance().create("SeekableChannel", properties, new MessageFactory())).create(tmpName)));
