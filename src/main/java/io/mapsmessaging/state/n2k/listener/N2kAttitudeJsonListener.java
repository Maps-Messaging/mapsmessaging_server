...
public class N2kAttitudeJsonListener extends AbstractN2kJsonListener {

  public static final int LISTENER_ID = N2kPgns.ATTITUDE;
  ...
  public void handle(DroneTwin droneTwin, JsonObject packet, TwinUpdateContext context) {
    Double rollRadians = getDouble(packet, "roll");
    Double pitchRadians = getDouble(packet, "pitch");
    Double yawRadians = getDouble(packet, "yaw");

    if (rollRadians == null && pitchRadians == null && yawRadians == null) {
      return;
    }

    Instant now = resolveTimestamp(context);

    droneTwin.setOrientation(new Orientation(
